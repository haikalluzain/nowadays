				...
				<div class="card shadow">
					<div class="card-body">
						<full-calendar ref="calendar" :config="config" @event-render="eventRender" :header="header"
							@day-click="dayClick" @event-selected="eventClick" :event-sources="eventSources"></full-calendar>
					</div>
				</div>
			</div>
		</section>
					...
					<form class="needs-validation" novalidate="">
						<div class="modal-body pb-0">
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									...
									<span class="invalid-feedback" v-show="errors.has('desc')">{{ errors.first('desc') }}</span>
								</div>
							</div>
							...
							<div class="form-group row">
								<div class="col-8">
									<v-date-picker is-inline mode='range' v-model='selectedDate' show-caps :formats="formats"
										:select-attribute="attrs" :theme-styles='themeStyles' :pane-width="280" @dayclick="cek"
									...
									<div class="row gutters-xs">
										<div class="col-auto">
											<label class="colorinput">
												<input type="checkbox" checked value="primary" id="colorPrimary" @click="colorChange('#6777ef')"
										...
										<div class="col-auto">
											<label class="colorinput">
												<input type="checkbox" value="danger" id="colorDanger" @click="colorChange('#fc544b')"
													class="colorinput-input">
										...
										<div class="col-auto">
											<label class="colorinput">
												<input type="checkbox" value="warning" id="colorWaring" @click="colorChange('#ffa426')"
													class="colorinput-input">
										...
										<div class="col-auto">
											<label class="colorinput">
												<input type="checkbox" value="info" id="colorInfo" @click="colorChange('#3abaf4')"
													class="colorinput-input">
										...
										<div class="col-auto">
											<label class="colorinput">
												<input type="checkbox" value="success" id="colorSuccess" @click="colorChange('#63ed7a')"
													class="colorinput-input">
						...
						<div class="modal-footer bg-whitesmoke br">
							<div class="mr-auto" v-show="dayEdit">
								<button class="btn btn-danger" @click.prevent="deleteEvent(event.id)"><i class="fas fa-trash"></i>
									Hapus</button>
							</div>
							<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
							<button v-if="!dayEdit" type="button" class="btn btn-primary" @click.prevent="addEvent">Submit</button>
							<button v-else type="button" class="btn btn-success" @click.prevent="updateEvent">Save changes</button>
						</div>
					</form>
				</div>
			</div>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<template v-if="!filtered.length">
							<div class="mt-4 alert alert-light">Data tidak ditemukan</div>
						</template>

						<template v-else>
							<ul class="mt-4 list-unstyled list-unstyled-border">
								<li class="media" v-for="event in filtered" :key="event.id">
									<div class="media-body">
										<div class="float-right text-muted">{{ moment(event.start).locale('id').format('LL') }}</div>
										<a href="#" @click.prevent="eventClick(event)"
											class="media-title text-primary">{{ event.title }}</a>
										<div class="div"></div>
					...
					<div class="modal-footer pt-0">
						<div class="m-auto">
							<button @click.prevent="openAddForm" class="btn btn-primary btn-shadow" id=""><i class="fas fa-plus"></i>
								New Event</button>
						</div>
					</div>
				</div>
	...
	export default {
				...
				eventSources: [
					{
						events(start, end, timezone, callback) {
							axios.get('/api/event/show', { timezone: timezone }).then((response) => {
		...
		methods: {
			...
			dayClick(date) {
				...
				this.selectedDate = { start: new Date(moment(date.format()).format("L")), end: new Date(moment(date.format()).format("L")) };
			...
			eventClick(event) {
				...
				this.selectedDate = { start: new Date(moment(this.event.start).format("L")), end: new Date(moment(this.event.end).format("L")) }
			...
			openAddForm() {
				this.fresh();
				this.selectedDate = { start: new Date(moment().format('L')), end: new Date(moment().add(2, 'days').format('L')) };
			...
			addEvent() {
				...
				if (this.event.title == '') {
					this.errors.add({ field: 'title', msg: 'Harap isi title dahulu!' })
		...
		computed: {
			...
			filtered() {
				return this.events.filter((events) => {
					...
					else if (events.title.match(this.filter)) {
						return events.title.match(this.filter)
					} else if (events.description.match(this.filter)) {
						return events.description.match(this.filter)
					} else if (events.start.match(this.filter)) {
						let start = moment(events.start).locale('id').format('MMMM')
						return events.start.match(this.filter)
					} else {
						return false
					}
				});
